...
export const isNavLinkActive = (link, router) => {
  ...
  const parts = router.currentRoute.value.name.split('-'); // Split the string by '-'
  const secondElementRoute = parts[1]; // clients

  // console.log("nuka1", resolveRoutedName);

  return matchedRoutes.some(route => {
    return route.name === resolveRoutedName || route.meta.navActiveLink === resolveRoutedName
    || resolveRoutedName.includes(secondElementRoute)
  })

  // (typeof link.to === 'string' && router.currentRoute.value.path.startsWith(link.to))
}
