/**
 * ISO 3166-1 alpha-2 countries for billing address / VAT determination.
 *
 * The country code is not cosmetic here: TaxService uses it to decide domestic
 * 20% VAT vs EU reverse charge vs non-EU zero-rating, so a typo in a free-text
 * field silently changes the tax on an invoice. Hence a fixed list.
 */

/**
 * EU member states — must stay identical to TaxService::EU_COUNTRIES in the API.
 * If one side changes (an accession or a withdrawal), change both.
 * @type {string[]}
 */
...
/** @param {string|null|undefined} code @returns {boolean} */
export const isEuCountry = code => EU_COUNTRIES.includes(String(code || '').toUpperCase())
...
export const COUNTRIES = [
  ...
  { code: 'SM', name: 'San Marino' }, { code: 'SA', name: 'Saudi Arabia' },
...
/**
 * Options for a v-select / v-autocomplete: EU members first (they decide VAT
 * treatment and are the common case for an Austrian seller), then the rest.
 * Titles carry the code so "AT" and "Austria" both match a search.
 *
 * @returns {{ title: string, value: string }[]}
 */
export const countryOptions = () => {
  const toOption = c => ({ title: `${c.name} (${c.code})`, value: c.code })
  const eu = COUNTRIES.filter(c => isEuCountry(c.code)).map(toOption)
  const rest = COUNTRIES.filter(c => !isEuCountry(c.code)).map(toOption)
