...
const fetchUserPreferences = async () => {
  try {
    const res = await $api('https://api-ds.bitemybytes.com/api/v1/user/preferences/fetch', {
      method: 'GET',
...
const onSwitchChange = async (item) => {
  ...
  try {
    const res = await $api('https://api-ds.bitemybytes.com/api/v1/user/preferences/store', {
      method: 'POST',
      body: jsonObject,
      onResponseError({ response }) {
