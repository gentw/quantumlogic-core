...
const fetchUserData = async () => {
  try {
    ...
    if(res.update_request == 1) {
      const rex = await $api('https://api-ds.bitemybytes.com/api/v1/user/profile/'+clientId+'/showUserRequestData', {
        method: 'GET',
        onResponseError({ response }) {
          // alert(1)
          // console.log("TEST", response);
      ...
      accountDataLocal.value.name = rex.name;
      accountDataLocal.value.surname = rex.surname;
      accountDataLocal.value.email = rex.email;
      accountDataLocal.value.phone = rex.phone;
      accountDataLocal.value.address = rex.address;
      accountDataLocal.value.city = rex.city;
      accountDataLocal.value.postal_code = rex.postal_code;
      ...
      if(rex.img == "/src/assets/images/avatars/avatar-1.png") {
        ...
        accountDataLocal.value.avatar_temp = 'https://api-ds.bitemybytes.com/' + rex.img;
      }
    } else {
...
const approveRequest = async () => {
  try {
    const res = await $api('https://api-ds.bitemybytes.com/api/v1/user/profile/'+clientId+'/approveUpdateUserRequest', {
      method: 'POST',
      onResponseError({ response }) {
    ...
    // console.log("TEST", res.name);
    fetchUserData();
    $toast.success('Shqyrtimi juaj u kry me sukses!');
...
const declineRequest = async () => {
  try {
    const res = await $api('https://api-ds.bitemybytes.com/api/v1/user/profile/'+clientId+'/declineUpdateUserRequest', {
      method: 'POST',
      onResponseError({ response }) {
        $toast.error('Ndodhi nje gabim!');      
...
const storeUserData = async () => {
  try {
    ...
    const res = await $api('https://api-ds.bitemybytes.com/api/v1/admin/updateClient/'+clientId, {
      method: 'POST',
      body: formData,
      onResponseError({ response }) {
        // alert(1)
    ...
    // console.log("TEST", res.name);
    fetchUserData();
    $toast.success('Klienti u perditesua me sukses!');
      ...
      <VCard>
        <VCardText>
          <h3 class="text-h3 text-medium-emphasis mb-4 text-normal">
            ...
            Profili i klientit: {{accountDataLocal.name}} {{ accountDataLocal.surname }}
            </span>
            <span v-else>
            Profili i klientit: {{accountDataLocal.name}} {{ accountDataLocal.surname }}
            </span>
          ...
          @submit.prevent="onSubmit">
            <VRow>
              ...
              >
                ...
                <div v-if="updateRequest == 0">
                  <VBtn type="submit">Ruaj</VBtn>
                </div>
                <div v-else>
                  <VBtn color="dark-btn" @click="approveRequest">
                    Prano kerkesen
                  </VBtn>

                  <VBtn class="ml-2" @click="declineRequest">
                    Refuzo kerkesen
                  </VBtn>
                </div>             
              </VCol>
