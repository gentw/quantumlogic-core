<script setup>
import useAdminList from '@/views/admin/users/admin/useAdminList';
...
const storeUserData = async () => {
  try {
    ...
    const res = await $api('https://api-ds.bitemybytes.com/api/v1/user/profile/updateUserRequest', {
      method: 'POST',
      body: formData,
...
const changePassword = async () => {
  try {
    const res = await $api('https://api-ds.bitemybytes.com/api/v1/user/profile/updatePassword', {
      method: 'POST',
      body: {
...
const onSubmit = () => {  
  refVForm.value?.validate().then(({ valid: isValid }) => {
    if (isValid)
      ...
      registerAdminFromAdmin(accountDataLocal.value).then(()=>{
...
">
            ...
            <VCardText class="d-flex flex-wrap gap-4">
              <VBtn class="w-100" type="submit">Shto admin të ri</VBtn>
            </VCardText>
          </VForm>
        </VCard>
      </VCol>
