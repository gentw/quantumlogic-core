...
- Backend (`api/app/`): no `Services/` layer yet — extracted business logic should land in `app/Services/<Name>Service.php`. Extracted validation belongs in `app/Http/Requests/<Name>Request.php`. Extracted authorization belongs in policies or middleware.
